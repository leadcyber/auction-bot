...
export default class AuctionEvents {
  ...
  onCloseAuction(auction) {
    this._auctionManager.closeAuction(auction._id).then((res) => {
      if (res) {
        const encryptedId = new Cipher().encrypt(
          auction._id.toString(),
          this._cipherPassword
        );
        this._telegram
          .sendMessage({
            ...
            reply_markup: {
              inline_keyboard: [
                [
                  {
                    text: `Pay`,
                    url: `${this._closeAuctionUrl}?token=${encryptedId}`,
                  },
                ],
              ],
            },
