...
export default class AuctionPictureCommand {
  ...
  execute(state, ...params) {
    let photos = params[0];
    if (photos) {
      let pict = photos.reduce((prev, cur) =>
        prev.file_size > cur.file_size ? prev : cur
      );
      ...
      return this._telegram
        ...
        .then((res) => {
          console.log(res);
          let nextState = state.single
            ? null
            : constants.STATE_WAIT_FOR_MIN_SUB;
          this._auctionManager
            .updateAuction(state.auctionId, {
              file_id: telegramResponse.file_id,
              image: res.Location,
            }) // res.Location
            .then((res) => {
