...
storageProvider
  ...
  .then((db) => {
    ...
    const auctionChant = AuctionChant(
      telegram,
    ...
    const auctionStartNotification = AuctionStartNotification(
      telegram,
      managerFactory
    );
    const auctionPayNotification = AuctionEndNotification(
      telegram,
      managerFactory
    );
    ...
    auctionTimer.schedule((ticks) => {
      if (ticks % 60 === 0) {
        ...
        auctionStartNotification.sendNotification(
          date,
          `${config.base_url}/pages/auction`
        );
      }
    });

    const closeAuctionUrl = config.base_url + urlConsts.PAGE_PAYPAL_GETPAYURL;
    const auctionEvents = new AuctionEvents(
      telegram,
      i18n,
      managerFactory.getAuctionManager(),
      closeAuctionUrl
    ...
    telegram.getMe().then((info) => {
      logger.info(
        `${
          info.result.first_name + " " + info.result.last_name
        } ready to answer!!`
      );
    });
